...
export default function LowStockPage() {
  ...
  const visibleRecords = useMemo(
    () =>
      showOtherYears
        ? lowStockRecords
        : lowStockRecords.filter((r) => r.yearGroup === "current" || r.yearGroup === "previous"),
    [lowStockRecords, showOtherYears]
  );
  const hiddenCount = lowStockRecords.length - visibleRecords.length;
  ...
  return (
    <main>
      ...
      <h1>低库存预警</h1>
      <p className="lead">专注查看低库存 SKU，可按需展开其他年份。</p>
      ...
      {visibleRecords.length > 0 && (
        <div className="card" style={{ margin: "12px 0" }}>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 12, alignItems: "center" }}>
            <div className="pill danger">低库存：{visibleRecords.length}</div>
            <div className="tag">
            ...
            {hiddenCount > 0 && (
              <button className="secondary" onClick={() => setShowOtherYears((v) => !v)}>
                {showOtherYears ? "隐藏其他年份" : `展开其他年份 (${hiddenCount})`}
              </button>
            )}
          </div>
        </div>
      ...
      <ExportButton weekId={weekId} records={visibleRecords} />
